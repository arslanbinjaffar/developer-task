...
const Index = () => {
  ...
  return (
      <div className='bg-security_icons bg-pacificblue px-8 py-14 '>
          <h2 className='text-4xl font-semibold text-white text-center '>SECURITY IS OUR PRIORITY</h2>
...
function Card({ image, title, description,scaleUp }: {
    image: string
    title: string
    description: string
    scaleUp?:number
    ...
    return (
        <div className='h-auto bg-white flex items-center flex-col rounded-2xl'
        style={{ transition: 'transform 0.5s', transform: `scale(${scaleUp})`}}
            >
            {/* img */}
            <Image src={image} width={112} height={112} alt='aws_logo'
            className='mt-10'
            />
            {/* heading */}
            <h3 className='text-ocean font-bold text-2xl text-center mt-5'>{ title}</h3>
